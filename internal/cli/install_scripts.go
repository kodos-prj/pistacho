...
func (i *InstallScriptsCommand) runInstallScriptDirect(pkg *registry.Package, operation string) error {
	// Path: {PoolRoot}/{name}/{version}/.INSTALL (typically ./pool/<name>/<version>/.INSTALL)
...
func (i *InstallScriptsCommand) runInstallScriptChroot(pkg *registry.Package, operation string, chrootDir string) error {
	// Execute script from root directory context, allowing relative paths in scripts to resolve correctly
	// When --chroot is used, the base-dir is typically /kod/pool (relative to the chroot)
	// This absolute path is correct within the chroot environment
	poolRoot := i.config.PoolRoot
	if !filepath.IsAbs(poolRoot) {
		// Convert relative paths to absolute for chroot context
		poolRoot = filepath.Join("/", poolRoot)
	}
	scriptPath := filepath.Join(poolRoot, pkg.Name, pkg.Version, ".INSTALL")
	shellCmd := fmt.Sprintf("cd / && source %s && %s", scriptPath, operation)
