...
import (
	...
	"github.com/kodos-prj/pistacho/internal/cli"
...
func handleDownload(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "Error: package name required")
		fmt.Fprintln(os.Stderr, "Usage: pith download <package> [package2] ...")
		os.Exit(1)
	}
...
func handleExtract(args []string) {
	if len(args) < 1 {
		...
		fmt.Fprintln(os.Stderr, "Usage: pith extract <package.pkg.tar.zst> [package2.pkg.tar.zst] ...")
		os.Exit(1)
	}
...
func handleInstall(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "Error: package name required")
		fmt.Fprintln(os.Stderr, "Usage: pith install [options] <package> [package2] ...")
...
func handleInstallScripts(args []string) {
	...
	for _, arg := range args {
		if arg == "--help" || arg == "-h" {
			fmt.Fprintln(os.Stderr, "Usage: pith install-scripts [options] [package ...]")
...
func handleList(args []string) {
	...
	for _, arg := range args {
		if arg == "--help" || arg == "-h" {
			fmt.Fprintln(os.Stderr, "Usage: pith list [options]")
			fmt.Fprintln(os.Stderr, "Options:")
...
func handleUpgrade(args []string) {
	...
	for _, arg := range args {
		if arg == "--help" || arg == "-h" {
			fmt.Fprintln(os.Stderr, "Usage: pith upgrade [options] [package ...]")
			fmt.Fprintln(os.Stderr, "Options:")
...
func handleCleanup(args []string) {
	...
	for _, arg := range args {
		if arg == "--help" || arg == "-h" {
			fmt.Fprintln(os.Stderr, "Usage: pith cleanup [options]")
			fmt.Fprintln(os.Stderr, "Options:")
...
func handleCache(args []string) {
	...
	for _, arg := range args {
		if arg == "--help" || arg == "-h" {
			fmt.Fprintln(os.Stderr, "Usage: pith cache [options]")
			fmt.Fprintln(os.Stderr, "Options:")
